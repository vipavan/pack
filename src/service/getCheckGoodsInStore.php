...
if ($conn) {
    ...
    try {
        ...
        $query = $cSysUser->getGetChkItemInStore();
        $param = array(&$GOODS_SKU,&$GOODS_NAME,&$GOODS_DESC,&$GOODS_TYPE,&$GOODS_BARCODE,&$GOODS_BRAND,&$GOODS_SERIES,&$GOODS_COLOR,&$GOODS_SIZE,&$GOODS_VOL,&$GOODS_VOL_UNIT,&$GOODS_PRODUCTION_DATE,&$GOODS_EXPIRED_DATE,&$CID);
        ...
        sqlsrv_commit($conn);
        if (empty($data)) {
            $return_val = array("status" => "1", "result" => 0 , "result2" => '');
            echo json_encode($return_val);
        } else {
            $return_val = array("status" => "1", "result" => $data, "result2" => $data2);
            echo json_encode($return_val);
        }
