...
//        $today = date("d");
        if (empty($data)) {
            $new = 1;
            $number = sprintf('%012d', $new);
            $goodsid = 'G' .$CID. $number;
            $query = $gen->getInsertNewMonth();
            ...
            if (!sqlsrv_execute($stmt)) {
                ...
                $lastRecord = substr($data[0]['DOC_INDEX'], -12);
                $num = intval($lastRecord) + 1;
                $number = sprintf('%012d', $num);
                $goodsid = 'G' .$CID. $number;
                $query = $gen->getUpdateIndex();
...
//        echo json_encode($goodsid);
        ...
        $param = array(&$CID, &$goodsid, &$GOODS_SKU, &$GOODS_NAME, &$GOODS_DESC, &$GOODS_TYPE, &$GOODS_QTY, &$GOODS_QTY_UNIT, &$GOODS_REMARK, &$GOODS_MIN, &$GOODS_TEMP,&$GOODS_BARCODE,&$GOODS_PIC_URL);
        $stmt = sqlsrv_prepare($conn, $query, $param);
        if (!$stmt) {
            throw new Exception("sqlsrv_prepare#" . $query . "#");
        }
...
//        echo json_encode("SUCCESS");
        ...
        $param = array(&$CID, &$goodsid, &$GOODS_BRAND, &$GOODS_SERIES, &$GOODS_COLOR, &$GOODS_SIZE, &$GOODS_VOL, &$GOODS_VOL_UNIT, &$GOODS_PRODUCTION_DATE, &$GOODS_EXPIRED_DATE, &$GOODS_PRICE);
        $stmt = sqlsrv_prepare($conn, $query, $param);
        if (!$stmt) {
            throw new Exception("sqlsrv_prepare#" . $query . "#");
        }
...
//        echo json_encode("SUCCESS");
        ...
        $param = array(&$CID, &$goodsid,  &$WH_ID, &$LOCATION_ID);
        $stmt = sqlsrv_prepare($conn, $query, $param);
        if (!$stmt) {
            throw new Exception("sqlsrv_prepare#" . $query . "#");
        }
...
//////////////////////////////////// insert Log Action ///////////////////////////////////
        ...
        $param = array(&$CID, &$ACTION,  &$loginid,&$goodsincome,&$goodsid,  &$GOODS_QTY, &$GOODS_QTY_UNIT,&$remark);
        $stmt = sqlsrv_prepare($conn, $query, $param);
        if (!$stmt) {
            throw new Exception("sqlsrv_prepare#" . $query . "#");
        }
