            ...
            <div class="grid_12">
                <div class="panel panel-default">
                    <div class="panel-heading" style="height:53px;">
                        <button type="button" class="btn btn-sm" onclick="window.location='pick.php'"
                    ...
                    <div class="panel-body">
                        <div style="text-align:center; font-size:30px;">
                            <label> ใบรายการ Pick สินค้า เลขที่ <?php echo $pick ?> </label> <br>
                            <div id="docDate"></div>
                        </div>
                        <input type="text" style="margin-top:20px;margin-bottom:20px;" id="search"
...
<script>
    $(document).ready(function () {
        ...
        function loadDate() {
            ...
            $.post('../../service/getDatePick.php', {pick: pick}).done(function (data) {
                var jsonData = JSON.parse(data);
//                alert(JSON.stringify(jsonData));
                var pDate = jsonData["result"][0]['PICKLOT_DOCDATE'];
                var str = pDate['date'];
                var subMFD = str.substr(0, 10);
                var fDate = subMFD.split("-");
                var reStr = fDate[2]+ '/' + fDate[1]+ '/' + fDate[0];
//                alert(reStr);
                var la = '<label> ประจำวันที่ ' + reStr +'</label>';
                $('#docDate').html(la);
            });
        }
...
//                alert(JSON.stringify(jsonData));
                ...
                if (status == 1) {
                    ...
                    for (i = 0; i < resL; i++) {
                        ...
                        append = append + "<td width='150' class=\"orderSum\"><a href=\"saleorder-detail.php?so="+ SO_DOCCODE +"&p="+pick+"\">รายละเอียด</td>";
