...
if(empty($_REQUEST['wh'])){
    $wh='';
}else{
    $wh = $_REQUEST['wh'];
}
if(empty($_REQUEST['lo'])){
    $lo='';
}else{
    $lo = $_REQUEST['lo'];
}
if(empty($_REQUEST['lod'])){
    $lod='';
}else{
    $lod = $_REQUEST['lod'];
}
                ...
                <div class="panel panel-default">
                    ...
                    <div class="panel-body">
                        <form class="form-horizontal" role="form">
                            ...
                            <div class="form-group">
                                <label class="control-label col-sm-4" for="">ชื่อโลเคชั่น</label>
                                <div class="col-sm-4">
                                    <input type="text" class="form-control" id="strlocation" placeholder="">
                                </div>
                            </div>
                            <!-- <div class="form-group">
                              <label class="control-label col-sm-4">ประเภทการจัดวาง</label>
                            ...
                            <!-- <div class="form-group">
                              <label class="control-label col-sm-4" for="">ราคา</label>
                              <div class="col-sm-4">
                                <input type="text" class="form-control" id="" placeholder="">
                              </div>
                                <label class="col-sm-1" for="">บาท</label>
                            </div> -->
                            <!-- <div class="form-group">
                              <label class="control-label col-sm-4" for="">รายละเอียด</label>
                              <div class="col-sm-4">
                            ...
                            <!-- <div class="form-group">
                              <label class="control-label col-sm-4" for="">ระยะเวลา</label>
                              <div class="col-sm-4">
                                <input type="text" class="form-control" id="" placeholder="">
                              </div>
                            </div> -->
                            <div class="form-group">
                                <div class="col-sm-offset-2 col-sm-5">
                                    <button type="button" onclick="window.location='setting.php#tab3default'"
                                            id="cancel" class="btn btn-info">ยกเลิก
                                    </button>
                                    <button type="button" id="savelocation" class="btn btn-info">ยืนยัน</button>
                                </div>
                            </div>
                        </form>
...
<script>
    $(document).ready(function () {
        var wh = "<?php echo $wh?>";
        var lo = "<?php echo $lo?>";
        var lod = "<?php echo $lod?>";
        loadWH(wh, lo,lod);
        $('#savelocation').click(function () {
            var warehouse = $('#strwarehouse').val();
            var location = $('#strlocation').val();
//            alert(warehouse+','+location);
            $.post('../../service/getCheckNameLocation.php', {whId: warehouse,loName:location}).done(function (data) {
                var jsonData = JSON.parse(data);
//                alert(JSON.stringify(jsonData));
                var res = jsonData["result"];
                if (res == 0) {
                    if(wh=="" && lo =="") {
                        $.post('../../service/insertNewLocation.php', {whId: warehouse,loName:location}).done(function (data) {
                            var jsonData = JSON.parse(data);
//                        alert(JSON.stringify(jsonData));
                            ...
                            if (statusRes == 1) {
                                if (res1 == "success") {
                                    ...
                                    window.location = "setting.php#tab3default";
                                }
                            }
...
//                        alert(JSON.stringify(jsonData));
                        ...
                        $.post('../../service/updateLocation.php', {whId: warehouse,loName:location,loId:lo}).done(function (data) {
                            var jsonData = JSON.parse(data);
//                        alert(JSON.stringify(jsonData));
                            var statusRes = jsonData["status"];
                            var res1 = jsonData["result"];
...
//                    alert(JSON.stringify(jsonData));
                    ...
                    if (statusRes == 1) {
                        ...
                        $("#strwarehouse").append(appendTxt);
                        $("#strwarehouse").prop("disabled",true);
                        $("#strwarehouse").val(wh);
                    }
                });
